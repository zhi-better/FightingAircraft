...
def load_and_render_tiled_map(xml_file_path, scale_factor=0.1):
    # 读取XML文件
    ...
    for y in range(height):
        for x in range(width):
            ...
            if tile_id != 0:
                rec_x = tile_id % 8 - 1 if tile_id % 8 != 0 else 7
                rec_y = tile_id // 8 - 1 if tile_id % 8 == 0 else tile_id // 8
...
# 使用函数加载地图
load_and_render_tiled_map("map/map0.xml", scale_factor=0.1)
